...
describe('getMyOrderitems', function() {
  ...
  it('should get all orders where dispatcher is equal to userId', async function() {
    const orders = [
        {
          id: '1',
          buyerId: '1',
      ...
      const result = await ordersQueryResolver.getTheOrderAsDispatcher(
        null,
        null,
        {
  ...
  it('should throw an error if an exception is raised', async function() {
    ...
    const result = await ordersQueryResolver.getTheOrderAsDispatcher(null, null, { user: { id: userId.toString() } });
